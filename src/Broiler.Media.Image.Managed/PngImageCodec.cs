...
{
    public static MediaCodecDescriptor CodecDescriptor { get; } = new(new MediaCodecId("broiler.image.png.managed"), "Broiler managed PNG/APNG",
        MediaKind.Image, MediaCodecCapabilities.Decode | MediaCodecCapabilities.Encode | MediaCodecCapabilities.Animation,
        [new MediaFormatDescriptor("PNG", ["image/png"], [".png", ".apng"])]);

    public PngImageCodec() : base(CodecDescriptor) { }
    ...
    public static ImageBuffer Decode(ReadOnlySpan<byte> data) => PngDecoder.Decode(data);

    public static ImageSequence DecodeAnimation(ReadOnlySpan<byte> data) => PngDecoder.DecodeAnimation(data);

    public static byte[] Encode(ImageBuffer buffer) => PngEncoder.Encode(buffer);

    public static byte[] EncodeAnimation(ImageSequence sequence) => PngEncoder.EncodeAnimation(sequence);
